...
class OptionalChannelElementsMixin(RSSBaseModel):
    ...
    "for example, on a single page."  # noqa
    ...
    "may be expressed with two characters or four characters (four preferred)."  # noqa
    ...
    "publish-subscribe protocol for RSS feeds."  # noqa
    ...
    "refreshing from the source."  # noqa
    ...
    "A hint for aggregators telling them which hours they can skip. This element contains up to 24 <hour> " \
    "sub-elements whose value is a number between 0 and 23, representing a time in GMT, when aggregators, if " \
    "they support the feature, may not read the channel on hours listed in the <skipHours> element. The hour " \
    "beginning at midnight is hour zero."  # noqa

    skip_days: Optional[str] = None
    "A hint for aggregators telling them which days they can skip. This element contains up to seven <day> " \
    "sub-elements whose value is Monday, Tuesday, Wednesday, Thursday, Friday, Saturday or Sunday. Aggregators " \
    "may not read the channel during days listed in the <skipDays> element."  # noqa
